...
Развёртываются **`N` одинаковых подов с leader election**: `N-1` подов работают
воркерами (порт 8081), а выбранный по Lease лидер **только управляет** — он
останавливает свой воркер, поднимает UI-контроллер (порт 8080), помечает себя
лейблом `role: leader` и исключает себя из списка воркеров, поэтому нагрузку не
генерирует. Суммарный целевой RPS делится между `N-1` воркерами. Сервис
`controller` маршрутизирует трафик только на лидера.
...
(`worker-0.worker-headless.loadtest:8081`, ...), лидер в списке отсутствует.

Механика элекшена: поды соревнуются за Lease `nt-demo-leader` (15 секунд
аренды, обновление каждые 3 секунды). Если лидер упал, лизы истекает, UI
переходит к другому поду, а бывший лидер снова становится воркером. Readiness
проверяется по отдельному health-эндпоинту (порт 9090), который работает на
всех подах независимо от роли. Требуется RBAC: `get/create/update` на
`coordination.k8s.io/leases` и `get/patch` на `pods`.
